...
- **llms.txt module**: zero-dependency parser for the [llms.txt specification](https://llmstxt.org/). `parse()` extracts structured data (title, description, details, H2 sections, Optional entries) from llms.txt files. `find_candidates()` provides unified URL discovery — searches parsed llms.txt entries (exact > extension variation > path prefix) with heuristic `.md` URL fallback when no llms.txt is available. Frozen dataclasses (`LlmsTxt`, `FileEntry`) for immutable parse results. 45 correctness tests + 4 benchmarks.
