...
|--------|---------|-----------|------|
| 简单日志 | 8.8 μs | 10.9 μs | 快 1.2x |
| 绑定上下文日志 | 10.2 μs | 15.4 μs | 快 1.5x |
| JSON 渲染 | 8.3 μs | 9.9 μs | 快 1.2x |
| 绑定 + 日志 | 10.0 μs | 17.0 μs | 快 1.7x |

## 要点总结

- **全场景快 1.2-1.7 倍** —— zerodep 在所有场景下均优于 structlog，上下文绑定操作优势更大。
- **简单日志和 JSON 渲染**场景下两者性能接近（约 8-11 μs），zerodep 快约 1.2 倍。
- **上下文操作优势最大** —— 绑定上下文日志（1.5x）和绑定+日志（1.7x）加速比最高，structlog 的处理器链和包装开销更明显。
