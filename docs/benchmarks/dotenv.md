...
|--------|---------|---------------|------|
| 小型（10 条目） | 26.5 μs | 23.0 μs | 慢 1.2x |
| 中型（50 条目） | 184.0 μs | 180.9 μs | ~1.0x |
| 大型（500 条目） | 1,140.6 μs | 1,139.4 μs | ~1.0x |

## 要点总结

- **中大型文件性能持平**——50 条目以上时，两者差异在测量噪声范围内。
- **小型文件略慢**——10 条目时 zerodep 约慢 1.2 倍，但绝对差异仅约 3.5 μs，实际使用中可忽略。
